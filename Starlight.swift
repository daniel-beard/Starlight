...
public class Starlight {
    ...
    private func calculateKey(inout u: State) -> State {
    ...
    private func getRHS(u: State) -> Double {
        guard u != s_goal else {
            return 0.0
        }
        if cellHash[u] == nil {
    ...
    private func getSucc(u: State) -> StateLinkedList {
        var s = StateLinkedList()
        var tempState = State()
    ...
    private func getPred(u: State) -> StateLinkedList {
        var s = StateLinkedList()
        var tempState = State()
    ...
    private func updateVertex(u: State) {
    ...
    private func isValid(u: State) -> Bool {
    ...
    private func getG(u: State) -> Double {
        if cellHash[u] == nil {
            return heuristic(u, b: s_goal)
        }
        return cellHash[u]!.g
    ...
    private func heuristic(a: State, b: State) -> Double {
        return eightCondist(a, b: b) * C1
    }
    
    /// Returns the 8-way distance between state a and state b
    private func eightCondist(a: State, b: State) -> Double {
    ...
    private func insert(var u: State) {
        u = calculateKey(&u)
    ...
    private func keyHashCode(u: State) -> Double {
    ...
    private func occupied(u: State) -> Bool {
    ...
    private func trueDist(a: State, b: State) -> Double {
    ...
    private func cost(a: State, b: State) -> Double {
        let xd = abs(a.x - b.x)
        let yd = abs(a.y - b.y)
        var scale = 1.0
    ...
    private func close(x: Double, y: Double) -> Bool {
        if x == Double.infinity && y == Double.infinity { return true }
        return abs(x - y) < 0.00001
    }
    
    /// Sets the G value for state u
    private func setG(u: State, g: Double) {
        makeNewCell(u)
        cellHash[u]!.g = g
    }
    
    /// Sets the rhs value for state u
    private func setRHS(u: State, rhs: Double) {
        makeNewCell(u)
        cellHash[u]!.rhs = rhs
    }
    
    /// Checks if a cell is in the hash table, if not it adds it in.
    private func makeNewCell(u: State) {
